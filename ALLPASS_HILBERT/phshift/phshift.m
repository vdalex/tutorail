...
phase = phfunc(params,freq1);	% compute the phase at all freqs for both legs (2xM).
...
op = optimset("MaxIter",4000, "MaxFunEvals", 5000, "TolX",1e-8, "Display", "iter");

while count
  % ---------------------------------------------------------------------------
  %  Estimate optimal parameters:
  [params, fval] = fminsearch(@phcost, params, op);  % find optimal parameters
  
  %[params, fval, exitflag, result] = fminsearch('phcost', params, optimset('MaxIter',4000, 'MaxFunEvals', 5000, 'TolFun',1e-12, 'TolX',1e-8, 'Display','on'), freqopt, phdesired, pnorm)  % find optimal parameters
