/*
	dbus_listener.c
	In this program we will use dbus_bus_add_match(DbusConnection *,const char *rule, DBusError *) to add a match 
	for the messages that we want to receive, the rule string has a specific format, see DBus match rule for full details.

	gcc dbus_listener.c -o dbus_listener `pkg-config --libs --cflags dbus-1 glib-2.0 dbus-glib-1`
*/
